...
*/                       
  ...
  (
    ...
    p_count   number; -- количество изменений в лююбимых номерах
    p_zone    zones.zone_id%type; -- зона абонента
    p_rtpl_id rate_plans.rtpl_id%type; -- идентификатор ТПО
  begin
    ...
    if sysdate between to_date('15.05.2007', 'DD.MM.YYYY') and
      ...
      --стоимость подключения либо изменение первых 3-х любимых номеров - бесплатно
      if P_COUNT < 4 then
        for rec in (select 1
