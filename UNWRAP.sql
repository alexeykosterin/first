...
for c in (select line, text from dba_source where type = 'PACKAGE BODY' and owner = 'BIS'
and name = 'BIS_NOTIFY_PG' order by line) loop
if c.line = 1 then 
b64_len := to_number(regexp_substr(regexp_substr(
c.text,'^[0-9a-f]+ [0-9a-f]+$',1,1,'m'),'[0-9a-f]+',1,2),'XXXXXXXXXX');
...
for i in 1 .. dbms_lob.getlength(tmp3) loop
  dbms_lob.write(tmp3,1,i,utl_raw.substr(
  charmap,utl_raw.cast_to_binary_integer(dbms_lob.substr(tmp3,1,i))+1,1));
end loop;
...
dbms_lob.append(t_tmp,hextoraw('0000000000000000'));
t_hd1 := utl_compress.lz_uncompress_open(t_tmp);
t_s1 := 1;
loop
begin
  utl_compress.lz_uncompress_extract(t_hd1,t_buffer);
  exception
...
if utl_compress.isopen(t_hd1) then
  utl_compress.lz_uncompress_close(t_hd1);
end if;
